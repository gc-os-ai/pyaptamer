...
__author__ = ["nennomp"]
__all__ = ["filter_words", "generate_triplets", "seq2vec"]
...
def seq2vec(
    ...
    Examples
    --------
    >>> from pyaptamer.utils._base import seq2vec
    >>> words = {'AA': 1, 'AC': 2, 'A': 3}
    >>> sequences = (['AAAC'], ['HHHC'])
    ...
    for seq, ss in zip(*sequence_list):
        output = []
        output_ss = []
        i = 0
    ...
    if outputs:
        ...
        for idx, (seq_array, ss_array) in enumerate(zip(outputs, outputs_ss)):
