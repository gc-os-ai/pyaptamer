...
class Solver:
    """
    Class implementing training logic via backpropagation for neural networks, for classification tasks.
    ...
    def load_best_model(self) -> None:
        ...
        if self.best_params is None:
            raise ValueError(
                "No best model state available. Train the model first."
            )
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            has_improved = (
                (monitor == "test_loss" and current_metric < best_metric) or
                (monitor == "test_accuracy" and current_metric > best_metric)
            )
            if has_improved:
