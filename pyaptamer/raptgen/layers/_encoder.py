...
residual `Bottleneck` blocks to extract sequence motifs , then max-pools over the sequence length to produce a single 
fixed-size hidden representation per sequence. 

   Parameters 
    ---------- 

    embedding_dim : int, optional, default=32 
        The dimensionality of the nucleotide embedding used throughout the residual blocks. Also the size of the output hidden representation. 
